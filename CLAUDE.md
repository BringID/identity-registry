...
- **Credential expiry**: per-credential-group `validityDuration` (seconds, 0 = no expiry). Set at `createCredentialGroup(id, validityDuration)` time and updatable via `setCredentialGroupValidityDuration()` (owner-only, affects future registrations only). On registration, `credentialExpiresAt[registrationHash] = block.timestamp + validityDuration` (skipped when 0). `removeExpiredCredential()` is public — anyone can call it after expiry to remove the commitment from the Semaphore group and clear registration state (`credentialRegistered`, `credentialExpiresAt`, `pendingRecoveries`). **Important: `registeredCommitments` is intentionally NOT deleted** — this enforces commitment continuity on re-registration (see below). Between expiry and removal, the credential technically still works (removal must be triggered).
- **Commitment continuity on re-registration**: When a credential expires and is removed, the user can re-register, but `registerCredential()` enforces that the same identity commitment must be used (`require(attestation_.semaphoreIdentityCommitment == previousCommitment)`). This prevents a **double-spend attack** where a user could change their commitment after expiry to obtain new Semaphore nullifiers for the same scope, effectively proving the same credential twice. By preserving the commitment, nullifier continuity is maintained across expiry cycles.
...
- **Recovery on expired+removed credentials**: `initiateRecovery()` works even after a credential has expired and been removed from the Semaphore group. It checks `registeredCommitments[registrationHash] != 0` (not `credentialRegistered`), so a user who lost their key after expiry can still recover. When the credential is already removed from Semaphore, `_executeInitiateRecovery()` skips the `SEMAPHORE.removeMember()` call. `executeRecovery()` re-sets `credentialRegistered = true` and adds the new commitment to the Semaphore group. **Recovery does NOT modify `credentialExpiresAt`** — key replacement and credential validity are independent concerns.
