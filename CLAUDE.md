...
- **Credential expiry**: per-credential-group `validityDuration` (seconds, 0 = no expiry). Set at `createCredentialGroup(id, validityDuration)` time and updatable via `setCredentialGroupValidityDuration()` (owner-only, affects future registrations/renewals only). On registration/renewal, `credentialExpiresAt[registrationHash] = block.timestamp + validityDuration` (skipped when 0). `removeExpiredCredential()` is public — anyone can call it after expiry to remove the commitment from the Semaphore group and clear registration state (`credentialRegistered`, `credentialExpiresAt`, `pendingRecoveries`). **Important: `registeredCommitments` is intentionally NOT deleted** — this enforces commitment continuity on renewal and enables recovery. Between expiry and removal, the credential technically still works (removal must be triggered).
- **Register vs Renew separation**: `registerCredential()` is strictly for first-time registration — it rejects calls where `registeredCommitments[registrationHash] != 0`. `renewCredential()` handles all subsequent activations: it requires the same identity commitment as the original registration, re-adds the commitment to Semaphore if it was removed, and resets the validity duration. Early renewal (before expiry) is allowed, effectively extending the credential. This prevents a **double-spend attack** where a user could change their commitment after expiry to obtain new Semaphore nullifiers for the same scope. Renewal is blocked during pending recovery (`require(pendingRecoveries[...].executeAfter == 0)`).
