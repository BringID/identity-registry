...
- **interfaces/IBringIDGated.sol** — Interface for `BringIDGated` consumers. Exposes `REGISTRY()`, `APP_ID()`, view functions `verifyProof()`, `verifyProofs()`, `getScore()`, and validation functions `validateProofRecipient()`, `validateProofRecipients()`. Errors: `WrongProofRecipient`, `ZeroRecipient`, `AppIdMismatch`.
